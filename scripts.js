...
function jellyfinCheckMixedContent(server) {
	// Previously this threw an error whenever the page was HTTPS and the
	// Jellyfin server was HTTP, to warn about mixed content. In practice
	// some setups (e.g. Tailscale IPs, browsers with insecure content
	// allowed for this site) can successfully reach an HTTP server from an
	// HTTPS page anyway, so we no longer block the attempt here — just let
	// the fetch happen and surface any real network error if it fails.
}
